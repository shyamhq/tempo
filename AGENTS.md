...
- **`stream-pump.ts spawnClaude` does not forward Tempo env vars explicitly to the child.** `pty-terminal.ts` sets `TEMPO_CONNECT_TOKEN` / `TEMPO_SESSION_ID` / `TEMPO_THREAD_ID` / `TEMPO_CONSOLE_URL` on the spawned process; `stream-pump.ts spawnClaude` only spreads `process.env` and merges `CLAUDE_CODE_ENABLE_TASKS: '0'`. Works today because the parent CLI process has them set, but stream-pump relies on ambient inheritance instead of explicit injection — an invisible coupling the pty driver doesn't share. Mirror the pty driver's explicit env block on `spawnClaude` the next time stream-pump.ts opens for a real reason. Spotted 2026-06-09 during the `--append-system-prompt-file` wiring code review.
