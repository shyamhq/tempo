...
- **`comments.anchor_offset_hint` column is dead schema.** Defined in `apps/console/db/schema.ts:91`; never written, never read. Was a remnant from the pre-BlockNote anchor model. Dropping it is a destructive migration; requires explicit Dev approval, deferred from the plan-comments-redesign PR. Schedule for a follow-up that asks the question explicitly. Filed 2026-06-07.
