...
- **`tempo_load_skill` is registered but missing from the `McpTool` enum.** `apps/worker/src/mcp/tools/load-skill.ts` registers `tempo_load_skill` and the shared Agent prompt (`packages/contracts/src/agent-prompt.ts`) names it as a mandatory tool, but `McpTool` in `packages/contracts/src/mcp.ts` (the 16-entry enum) omits it. The tool works at runtime (registration is independent of the enum), so this is a contract-completeness gap, not a live bug. Add `'tempo_load_skill'` to the enum the next time `mcp.ts` is touched — it's a contracts change (judge-gateable), so not drive-by'd here. Filed 2026-06-19 during the Agent-prompt consolidation.
- **`tempo_poll` still in the `McpTool` enum.** `mcp.ts` line ~160 lists `'tempo_poll'`, but the in-flight redis-streams-realtime work removes `tempo_poll` from both the MCP server and contracts (events are pushed, not polled). The consolidated Agent prompt already omits any `tempo_poll` reference. When the redis-streams branch merges, the enum entry and `apps/worker/src/mcp/tools/poll.ts` must be dropped together so the enum stays the authoritative tool surface. Filed 2026-06-19.
...
- **`formatTime` duplicated across discussion/editor.** `apps/console/components/thread/discussion/message-list.tsx` now exports `formatTime(iso: string)` and is consumed by `question-card.tsx`. `apps/console/components/thread/editor/plan-comment-card.tsx` carries its own private `formatTime(d: Date)` with a `Date` signature instead of an ISO string. Same `toLocaleTimeString` call inside. Two callers each → not yet worth a `lib/format-time.ts`; consolidate the day a third caller appears or the next time both files are open in the same change. Filed 2026-06-17 with the C1 chat-row redesign.
...
- **No-VM conversation runtime's abort/stall/lock-lost branches are untested.** Spotted 2026-06-21 during the heartbeat+abort code review. `apps/worker/src/hosted/conversation.ts` gained a heartbeat (lock-lease refresh) + stall watchdog + abort handling (`runStreamTurn` returns `'done'|'stalled'|'lock-lost'`). `conversation.test.ts` covers only the happy path + lock lifecycle; the abort branches have no coverage because the `setInterval(HEARTBEAT_MS=60s)` never ticks under the instant-resolving `streamText` stub — exercising them needs fake timers + a hanging stream stub (or injectable `HEARTBEAT_MS`/`STALL_MS`). Consistent with the MVP "no tests" baseline (T12); add a focused test (mock `refreshTurnLock`→`false` under fake timers for lock-lost; advance past `STALL_MS` with a silent stream for stalled) if this path ever misbehaves. The `@tempo/server` test mock already registers + resets `refreshTurnLock`/`postReply`/`updateThread`; add them to the `ServerMock` handle/type when a test needs to drive them.
...
- **Hosted runner emits `session_connected` before `buildToolset` completes.** `apps/worker/src/hosted/runner.ts:304` posts the event right after `maybeCloneRepo` succeeds; the MCP transports negotiate after that. If `buildToolset` errors (filesystem stdio MCP can't start, Tempo MCP HTTP handshake fails), Console has already shown the session as connected. Acceptable today — both transports start in process and rarely fail post-clone — but the right invariant is "connected = runner ready to take a Turn". Move the emit to after `buildToolset` resolves the day this surface is next touched. Surfaced 2026-06-16 by code-simplifier during the turn-input-bloat-cut review.
- **MCP write tools are round-trips the agent doesn't need.** After the 2026-06-17 convergence refactor, the agent receives all state pre-injected (Turn 1 via `context`, Turn 2+ via `events`) and never needs to pull data. The remaining MCP tool calls are mostly writes — `tempo_post_reply`, `tempo_post_discussion_message`, `tempo_update_plan`, `tempo_update_block`, `tempo_add_blocks`, `tempo_delete_block`, `tempo_set_thread_meta` — all fire-and-forget. For these, MCP is unnecessary overhead: the agent could emit a structured action tag in its text stream, and the CLI runner (already reading the stream in `stream-pump.ts`) intercepts and makes the HTTP call directly. The two read tools (`tempo_pull_plan`, `tempo_load_skill`) could be eliminated too — plan state is pre-injected on `plan_edited_by_dev` events, and skills could be fetched by the runner and embedded in the event payload before injection. The natural convergence point is the Vercel AI SDK pattern the hosted runner already uses (SDK tool loop replaces MCP), which would mean the CLI agent runs via Anthropic SDK rather than the `claude` subprocess. This makes both paths architecturally identical: runner owns the tool dispatch loop, agent emits tool intents, no MCP protocol involved. Suitable when the CLI agent is ready to move off the `claude` subprocess. Spotted 2026-06-17 during the agent bootstrap convergence discussion.
- **Stale presence on unclean CLI exit.** `apps/worker/src/server/presence.ts` is a plain in-memory `Map<threadId, Set<connId>>`. Under the 2026-06-17 long-poll architecture, the CLI registers presence per-request (the gap between polls is ~milliseconds so the 5-second presence-check interval never detects it as disconnected). If the CLI crashes mid-session without a clean shutdown, the `connId` entry stays in the Map until the Worker process restarts, leaving the Console presence chip stuck "connected". Fix: add a TTL-based expiry to `presence.ts` (key + last-seen timestamp, sweep on each `addConnection` call). Spotted 2026-06-17 during the SSE→long-poll convergence review; deferred as MVP-acceptable.
