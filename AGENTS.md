...
- **`apps/agent` `dev` script runs the built `dist/cli.js` under Node, not `src/cli.ts` under Bun.** This is deliberate: `node-pty` data events do not fire under Bun's event loop, so the published CLI (Node-only, per package `engines`) is also the dev surface. If a future change adds non-PTY paths that benefit from Bun's faster iteration, restore a Bun-runtime `dev` script separately.
...
- **A displaced Agent's session row stays at `connected` until its next auth-failing call.** `createSessionFromToken` marks the prior connected session `disconnected` only when a *new* Agent connects (`sessions.ts:30-48`). If the local Agent dies without disconnecting and the Dev doesn't reconnect, the row sits stale. Pre-existing; surfaced again while wiring the "Connect" button (2.15). Fix by adding a `last_seen_at` heartbeat plus a sweep, or by having `authFromRequest` mark the session disconnected when the bearer no longer matches.
- **2026-06-05 — `TEMPO_AGENT_DRIVER` A/B flag (`pty` default vs `stream-json`).** Same shape as the cleaned-up `TEMPO_AGENT_LOOP` precedent below. Lives in `apps/agent/src/env.ts` + a one-line branch in `connect.ts`. The `stream-json` driver (`apps/agent/src/stream-pump.ts`) spawns `claude -p --output-format stream-json` per turn, captures `agent_narration` events the PTY's PreToolUse hook can't see. When the comparison settles, delete the losing arm and the env-var branch — same dispatch-row cleanup pattern as 2.19.
...
- **`PlanSaveBar.save` failure has no UI surface.** `usePlanSave.save()` catches `persist` rejections silently and leaves the bar visible. The bar reads "Unsaved changes" in both the never-saved and save-failed states, with no signal that the last attempt failed. Acceptable for MVP solo-Dev with the Console open in the same browser (the failure is usually network-down, the Dev can see); fix by passing the failure into a future toast primitive (see existing "Console has no toast primitive" note) or by rendering an error state on the bar itself. Surfaced 2026-06-03 by code-reviewer.
...
- **`PlanEditor key={discardKey}` remount drops in-progress comment composer.** Discard now confirms via `window.confirm`, but if the user has the comment composer open and confirms discard, the composer is torn down without warning. Pre-existing design of using `key` to reset stateful subtrees; not a regression. Fix when next touching the discard flow — either close the composer explicitly before bumping `discardKey`, or warn in the confirm prompt. Surfaced 2026-06-03 by code-reviewer.
