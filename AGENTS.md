...
- **`EventKind` enum in `packages/contracts/src/events.ts` duplicates the discriminator literal list from the `Event` union.** Adding a new kind requires two edits and a silent drift is possible. Acceptable today (the union is small); revisit if the count grows or a drift bug bites.
