...
- **`TEMPO_MIDTURN_HOOK` A/B flag must be retired after the experiment.** Introduced in 2.13. The PostToolBatch hook in `apps/agent/src/post-tool-batch-hook.ts` is only registered when `TEMPO_MIDTURN_HOOK=1`; default off keeps the Stop-hook-only path. When A/B is concluded, pick one and delete the other: **keep mid-turn** → drop the env-var guard in `spawn-claude.ts`'s `hookSettingsJson()` so PostToolBatch is always registered (Stop hook stays as the turn-boundary safety net); **drop mid-turn** → delete `post-tool-batch-hook.ts`, the dispatch case in `cli.ts`, and the conditional block in `spawn-claude.ts`. Either way, remove this note.
