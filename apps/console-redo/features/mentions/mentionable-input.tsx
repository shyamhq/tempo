...
// The @mention-aware composer field: a contenteditable that renders picked
// mentions as non-editable tokens and serialises to plain text + the resolved
// Mention[] sidecar (mention tokens become `@Label` in the text, deduped in
// document order). Ported from apps/console — the contenteditable handling
// (anchor span, caret math, portal-positioned picker) is load-bearing and kept
// faithfully; only the chrome is re-tokened to console-redo's --tp-* kit.
//
// The picker is portalled to document.body and positioned via the input's
// bounding rect so it escapes any overflow:hidden ancestor (the BlockNote
// FloatingThread popover, the rounded comment card). Cmd/Ctrl+Enter submits.
  ...
  for (const node of el.childNodes) {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (span.dataset.mentionId !== undefined) {
        ...
        const kind = span.dataset.mentionKind;
        // Skip a span whose kind isn't a known Mention kind rather than emit a
        // malformed Mention.
...
function MentionOverlay({ query, candidates, anchorRef, onSelect, onDismiss }: OverlayProps) {
  ...
  // Arrow/Enter/Escape are captured at document level so they intercept before
  // the composer's own keydown handler.
  useEffect(() => {
  ...
  // overflow:hidden ancestor. Recomputed on open + on scroll/resize.
  ...
  return createPortal(
    <div
      ...
      style={{
        position: 'fixed',
        left: pos.left,
        // Anchor the picker's bottom to the input's top (grows upward);
        // translateY(-100%) keeps it tight against the input at any height.
        top: pos.top,
        width: pos.width,
        transform: 'translateY(-100%) translateY(-4px)',
        zIndex: 1000,
      }}
      className="overflow-hidden rounded-lg border border-border-strong bg-canvas shadow-lg"
    >
      {filtered.map((c, i) => (
        <button
          key={c.id}
        ...
        >
          <span
            className={`inline-flex size-6 shrink-0 items-center justify-center rounded-full font-sans text-2xs font-semibold ${
              c.kind === 'agent' ? 'bg-primary-soft text-primary' : 'bg-inset text-ink-3'
            }`}
          >
          ...
          {c.kind === 'agent' ? (
            <span className="ml-auto font-mono text-2xs text-ink-3">Agent</span>
          ) : null}
        </button>
      ))}
    </div>,
...
type Props = {
  ...
  maxHeight?: number;
  className?: string;
  /** Called on Cmd/Ctrl+Enter when the picker is closed and there is content. */
...
export const MentionableInput = forwardRef<MentionableInputRef, Props>(function MentionableInput(
  ...
  // Insert a mention token at the cursor, removing the preceding `@query` anchor.
  const insertMention = useCallback(
    (candidate: MentionCandidate) => {
      ...
      // Place the cursor after the trailing space.
  ...
  const handleInput = useCallback(() => {
    ...
    if (!picker.open) {
      ...
      try {
        // surroundContents throws HierarchyRequestError when the range crosses
        // an element boundary; dismiss the picker rather than crash.
      ...
      // Restore the cursor to the end of the text inside the anchor span.
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      // Cmd/Ctrl+Enter submits — but only when the picker is not consuming Enter.
  ...
  const handleBlur = useCallback(
    (e: React.FocusEvent<HTMLDivElement>) => {
      // Keep the picker open if focus moved into the overlay.
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        onBlur={handleBlur}
        className={`block w-full overflow-y-auto bg-transparent font-sans text-sm leading-body text-ink outline-none ${className ?? ''}`}
