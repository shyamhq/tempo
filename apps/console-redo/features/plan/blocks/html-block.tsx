...
// Client React variant of the `htmlBlock`, registered in the plan schema
// (`features/plan/schema.ts`) for the live editor surface. `@blocknote/react`
// calls `createContext` at module load — so the entire transitive import graph
// from here is client-only. Both this and the server-safe vanilla spec in
// `@tempo/server` share `html-block-shared.ts` so they agree on type /
