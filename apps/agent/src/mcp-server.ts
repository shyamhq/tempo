...
export async function runStdioMcpServer(args: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Replace the targeted block with the HTML. Provide the block id from tempo_pull_plan and the new HTML. The HTML may parse to one or more top-level blocks (each `<li>` is its own block, so a list with N items is N blocks). The first parsed block replaces the slot and keeps the original id, so Comments anchored to *this block itself* survive; any additional blocks insert right after with new ids. Comments anchored to inline text inside the replaced block may surface in the Dev\'s Orphaned list — that is expected. Call tempo_pull_plan after to see the new ids if you need to address inserted blocks later. For Mermaid diagrams emit `<pre><code class="language-mermaid">…</code></pre>` — the Console renders these as live SVG diagrams; without the `language-mermaid` class they render as a plain code block. For callouts emit `<div class="alert alert-warning">…inline html…</div>` (or `alert-error`, `alert-info`, `alert-success`) — the Console renders these as styled callout blocks; preserve the second class on rewrites or the variant is lost. For embedded HTML pages emit `<pre><code class="language-html-block">…html…</code></pre>` — the Console renders these as a sandboxed iframe; without the `language-html-block` class they render as a plain code block.',
  ...
  server.registerTool(
    ...
    {
      description:
        "Insert new blocks. `reference_id` is the id of an existing block (or null + position:'end' to append). `position` is 'before', 'after', or 'end'. `blocks` is an array of HTML strings; each string may parse to one or more top-level blocks (each `<li>` is its own block) and entries are flattened in array order before insertion. The returned `ids` cover every inserted top-level block and may exceed the length of `blocks`. For Mermaid diagrams emit `<pre><code class=\"language-mermaid\">…</code></pre>` — the Console renders these as live SVG diagrams; without the `language-mermaid` class they render as a plain code block. For callouts emit `<div class=\"alert alert-{warning|error|info|success}\">…inline html…</div>` — the Console renders these as styled callout blocks (warning / error / info / success); preserve the variant class on rewrites or the styling is lost. For embedded HTML pages (design-system surfaces, interactive prototypes) emit `<pre><code class=\"language-html-block\">…html…</code></pre>` — the Console renders these as a sandboxed iframe; without the `language-html-block` class they render as a plain code block.",
