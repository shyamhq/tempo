...
// client via `useOrganization()` / `useOrganizationList()` — no GET endpoint
// duplicates that data. This file owns the admin-only mutations that go
// through the Clerk Backend SDK.
...
export async function PATCH(req: NextRequest) {
  ...
  if (!parsed.data.name) return ok({ ok: true });
