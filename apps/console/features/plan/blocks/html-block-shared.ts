// Shared shape for the htmlBlock — the type / propSchema / content / parse the
// client React spec (`html-block.tsx`) registers in the plan schema. The
// server-safe vanilla variant lives in `@tempo/server` and must agree with this
// shape for PM JSON to round-trip, so the agreement is hosted here.
//
...
// This is the contract with the Agent and with stored PM JSON — it cannot change
// once shipped.
//
// The HTML source is stored on `props.html`, not as BlockNote inline content:
// `content: 'inline'` made PM normalise multi-line text (splitting at `\n`,
// dropping nodes) and Ctrl+A inside the source surface escaped the block. A prop
// + Dev-owned source view keeps raw HTML lossless and scopes selection.
